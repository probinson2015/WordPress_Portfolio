...
class Sela_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Sela_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					__( 'The %1$s is required to use some of this theme&rsquo;s features, including: ', 'sela' ),
					'<strong>' . __( 'Jetpack plugin', 'sela' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => __( 'no specific module needed', 'sela' ),
			'custom-content-types' => __( 'Custom Content Types module', 'sela' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'sela' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'sela' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'sela' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'sela' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'sela' ),
...
add_action( 'admin_head', array( 'Sela_Theme_Plugin_Enhancements', 'init' ) );
